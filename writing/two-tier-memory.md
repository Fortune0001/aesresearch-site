...
You don't see it happen. You see the output. The output is fine until it isn't. By the time the drift is visible, the agent is operating on summaries it can no longer separate from facts.
...
The root issue: chat memory treats every utterance as homogeneous. Production agent memory needs to distinguish between *this is what I'm thinking about right now* and *this is a fact I shouldn't have to rediscover next session*. Those are not the same data structure.
...
- **Feedback memory** — guidance the user has given about how the agent should work. Both corrections and validated choices. "Integration tests must hit a real database, not mocks — prior incident where mock/prod divergence masked a broken migration." (That "Why:" line does the real work. More below.)
