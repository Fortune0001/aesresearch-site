...
A hierarchical research swarm, built on AES Research's investigation architecture, investigated the **entire Ray codebase — 1.37M lines across three languages (C++, Python, Java), 442 of its 719 core directories (780K lines) investigated in depth — and produced a single coherent architecture-level synthesis that stitches findings *across the language boundary*, with no individual reasoning context ever exceeding a bounded working set.**
...
The headline result is a cross-language architectural claim no single-language pass could produce: that Ray funnels all distributed computation — across three languages and the five Ray libraries plus adjacent subsystems — through exactly three C++ kernel primitives.
...
| Scale (measured) | **1,370,296 total lines** (code + comment + blank, core source dirs) — Python 1.10M · C++ 235K · Java 33K; 719 core source dirs |
...
3. **The Controller–Worker FSM as a recurring structural motif.** Every library independently re-derives the same topology (a singleton controller actor + a pool of worker actors): Tune's `TuneController`, Serve's `ServeController`, Train's `TrainController`, Data's pipeline coordinator. The kernel never imposes it — it's the natural "eigenmode" of an actors-plus-futures primitive set.
...
5. **The Python library ecosystem as an uninstrumented composition layer.** Ray's highest-value code (Tune, Serve, Data, Train, RLlib) is **entirely invisible to the C++ kernel** — which sees only actor failures, object losses, and node deaths, never "training trial diverged" or "serve replica errored." Greatest strength (kernel simplicity) and biggest observability gap (no built-in bridge between the kernel view and the library view) at once.
6. **The GCS as metadata bus — a single point of coordination, held off the data path.** GCS is the sole cluster-wide metadata authority (actor registry, placement groups, resources) while deliberately staying *off* the data path (which flows through Plasma shared memory). That metadata/data split is why object throughput scales independently of coordination throughput, and no library bypasses it — a hard invariant of the *coordination* architecture. It is not a fault-tolerance property, and the investigation's original phrasing ("not of failure") overstated it: Ray's own docs are explicit that by default the GCS isn't fault tolerant and that if it fails, the entire Ray cluster fails. Making it non-single-point-of-failure means opting into an HA Redis backing — officially supported on KubeRay, self-supported elsewhere.
7. **Java bindings as structural proof of kernel minimality.** The Java binding is a thin JNI/protobuf adapter with no logic of its own, and supporting it required only small, well-isolated kernel-side additions (a language enum, some per-language handling) rather than reworking the core — strong evidence the kernel API is genuinely language-agnostic. Its symmetry with the Python binding shows Ray's language-neutrality is achieved, not aspirational.

> *"Ray is a polyglot distributed runtime whose entire architectural power derives from a single strategy: funnel all distributed computation — across three languages and the five Ray libraries plus adjacent subsystems — through exactly three C++ kernel primitives (task, actor, ObjectRef), mediated by protobuf/gRPC at every language boundary, so that the kernel remains the sole arbiter of scheduling, fault tolerance, and object lifecycle."* — master synthesis, converged across all 3 mid-coordinators with zero dissent.
...
- **Every figure is measured, not extrapolated.** Line counts are total physical lines (code + comment + blank) over the core source trees, counted from a fresh clone at the pinned commit (cloc code-only lines are ~923K); investigation targets were the 442 core directories with ≥100 LOC; agents fetched each directory directly from GitHub `master` URLs; per-launch usage was logged, and every figure above is measured from the clone or counted from the 18 sub-investigation logs.
...
- GCS fault tolerance (default behaviour and HA Redis): <https://docs.ray.io/en/latest/ray-core/fault_tolerance/gcs.html>
