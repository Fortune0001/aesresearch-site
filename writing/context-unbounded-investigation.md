...
| 2 | `kubernetes/kubernetes` | Go | 3.6M LOC; 2,001 packages investigated | Generalizes across domain — non-AI, distributed-systems, a different language |
| 3 | `ray-project/ray` | C++ / Python / Java | 1.37M LOC; 442 core directories | Generalizes across *languages in a single run* — synthesis that stitches findings across the language boundary |
...
**Transformers** resolved into a registry-driven, configuration-first federation: ~450 self-contained model packages held coherent not by deep inheritance but by a small set of contracts — the configuration object as the true public API, a backbone-plus-task-head seam as the universal composability point, containment (not inheritance) for multimodal composition, and a quiet architectural fact with a future: the contract does not privilege attention, which is why FFT cores, state-space models, and linear-recurrence models already live in the library without breaking it. Six cross-cluster patterns, all 15 subs blind to one another; 117 confirmed hypotheses, 1 refuted.

**Kubernetes** resolved into a single architectural cell stamped out at scale: every API resource traverses the same define → generate → cache → reconcile pipeline, the watch-reconcile loop recurs fractally at five scales, and ~8 code-generator templates — not the ~2,000 packages — are the real codebase. All 12 mid-syntheses converged on that account with zero dissent; 494 confirmed hypotheses, 12 refuted, 0 left open.
